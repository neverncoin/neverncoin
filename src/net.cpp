...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Neverncoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"5.83.163.241", "neverncoin.freeddns.org"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Neverncoin is probably already running."), addrBind.ToString().c_str());
        else
