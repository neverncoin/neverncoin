...
#if defined(NDEBUG)
# error "Neverncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x001cda16f9ae8a91e5544219d243c61ceeef12fc5fca26efe4af0c77a12a4127");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Neverncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Neverncoin Signed Message:\n";
...
{
    // Neverncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Neverncoin: 210k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Neverncoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Neverncoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Neverncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Neverncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xced3e466a5a96a5c787c5ae41c56b12316005375cab651735343df554cd7c2ca");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "TR Times 30 December 2020 Badem Dies";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0413494c38d2b95ea9b651ca921e076226b6fa831666538c7e1f5bfe9a4922de4cb4292a9d97eab4e19bf72a1d5a3dc1695c566f5623fcccdf63b85839c2cbb303") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1609689693;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084770338;

        if (fTestNet)
        {
            block.nTime    = 1609689666;
            block.nNonce   = 390493471;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb2878c402d03fa8a5a6837f88abed5e27cf1d79c0f0f592c1bec0db735534915"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xb3, 0xda }; // Neverncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NeverncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NeverncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NeverncoinMiner(CWallet *pwallet)
{
    printf("NeverncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("neverncoin-miner");
    ...
    try { loop {
        ...
        printf("Running NeverncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NeverncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NeverncoinMiner, pwallet));
}
